...
template <class mint,
          int g = primitive_root<mint::mod()>,
          is_static_modint_t<mint>* = nullptr>
struct fft_info {
    static constexpr int rank2 = lower_bit_constexpr(mint::mod() - 1);
    ...
    while (len < h) {
        if (h - len == 1) {
            ...
            for (int s = 0; s < (1 << len); s++) {
                ...
                if (s + 1 != (1 << len))
                    rot *= info.rate2[lower_bit(~(unsigned int)(s))];
            }
            len++;
        } else {
            // 4-base
            ...
            for (int s = 0; s < (1 << len); s++) {
                ...
                if (s + 1 != (1 << len))
                    rot *= info.rate3[lower_bit(~(unsigned int)(s))];
            }
            len += 2;
        }
    }
    ...
    while (len) {
        if (len == 1) {
            ...
            for (int s = 0; s < (1 << (len - 1)); s++) {
                ...
                if (s + 1 != (1 << (len - 1)))
                    irot *= info.irate2[lower_bit(~(unsigned int)(s))];
            }
            len--;
        } else {
            // 4-base
            ...
            for (int s = 0; s < (1 << (len - 2)); s++) {
                ...
                if (s + 1 != (1 << (len - 2)))
                    irot *= info.irate3[lower_bit(~(unsigned int)(s))];
            }
            len -= 2;
        }
    }
